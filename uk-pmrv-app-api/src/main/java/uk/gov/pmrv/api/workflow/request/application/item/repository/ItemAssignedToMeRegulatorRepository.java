...
@Repository
public class ItemAssignedToMeRegulatorRepository {

    @PersistenceContext
    private EntityManager entityManager;

    public ItemPage findItemsAssignedTo(String userId, Map<CompetentAuthority, Set<RequestTaskType>> scopedRequestTaskTypes,
        ...
        scopedRequestTaskTypes.forEach((ca, types) ->
            hasCaPermissionOnTask
                .or(request.competentAuthority.eq(ca).and(requestTask.type.in(types)))
        ...
        JPAQuery<Item> jpaQuery = query.select(
                ...
                .where(requestTask.assignee.eq(userId).and(hasCaPermissionOnTask))
    ...
    public ItemPage findItemsAssignedToByAccount(String userId, Long accountId, Map<CompetentAuthority, Set<RequestTaskType>> scopedRequestTaskTypes,
                                                 Long page, Long pageSize) {
        QRequest request = QRequest.request;
        QRequestTask requestTask = QRequestTask.requestTask;
        QRequestTaskVisit requestTaskVisit = QRequestTaskVisit.requestTaskVisit;
        ...
        scopedRequestTaskTypes.forEach((ca, types) ->
            hasCaPermissionOnTask
                .or(request.competentAuthority.eq(ca)
                    .and(requestTask.type.in(types).or(request.type.eq(RequestType.SYSTEM_MESSAGE_NOTIFICATION))))
        );
