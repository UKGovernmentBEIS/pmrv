...
@Repository
public class ItemAssignedToOthersVerifierRepository {

    @PersistenceContext
    private EntityManager entityManager;

    public ItemPage findItemsAssignedToOthers(String userId, Map<Long, Set<RequestTaskType>> scopedRequestTaskTypes,
        ...
        scopedRequestTaskTypes.forEach((vbId, types) ->
            hasPermissionOnTask.or(request.verificationBodyId.eq(vbId).and(requestTask.type.in(types)))
        );
        ...
        JPAQuery<Item> jpaQuery = query.select(
                ...
                .where(requestTask.assignee.ne(userId).and(hasPermissionOnTask))
    ...
    public ItemPage findItemsAssignedToOthersByAccount(String userId, Long accountId, Map<Long, Set<RequestTaskType>> scopedRequestTaskTypes,
                                                       Long page, Long pageSize) {
        QRequest request = QRequest.request;
        QRequestTask requestTask = QRequestTask.requestTask;
        QRequestTaskVisit requestTaskVisit = QRequestTaskVisit.requestTaskVisit;
