...
public class N2OMonitoringApproachSectionValidator implements
    PermitSectionContextValidator<N2OMonitoringApproach>, PermitContextValidator, PermitGrantedContextValidator {
    ...
    public PermitValidationResult validate(PermitContainer permitContainer) {
        ...
        if(monitoringApproaches.containsKey(MonitoringApproachType.N2O)) {
            return validate((N2OMonitoringApproach) monitoringApproaches.get(MonitoringApproachType.N2O), permitContainer);
        }
        return PermitValidationResult.validPermit();
    }

    @Override
    public PermitValidationResult validate(N2OMonitoringApproach permitSection, PermitContainer permitContainer) {
        ...
        List<PermitViolation> permitViolations = permitSection.getSourceStreamCategoryAppliedTiers().stream()
            .map(appliedTier -> Stream.of(
                ...
                return new PermitViolation(N2OMonitoringApproach.class.getSimpleName(),
                                           p.getLeft(),
                                           data);
