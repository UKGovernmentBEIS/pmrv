    ...
    map((account) =>
      this.fb.group({
        sopId: [
          account?.sopId,
          [
            GovukValidators.required('The ID should contain numbers only. Try entering the ID again.'),
        ...
        switchMap(([form, account]) =>
          form.dirty
            ? this.accountUpdateService
                .updateAccountSopIdUsingPOST(account.id, {
                  sopId: form.get('sopId').value,
                })
                .pipe(tap((this.route.snapshot.data.account.sopId = form.get('sopId').value)))
            : of(null),
        ),
      )
