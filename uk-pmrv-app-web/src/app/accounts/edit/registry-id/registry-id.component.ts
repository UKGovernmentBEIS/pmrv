    ...
    map((account) =>
      this.fb.group({
        registryId: [
          account?.registryId,
          [
            GovukValidators.required('Enter a 7 digit number'),
            GovukValidators.min(1000000, 'Enter a 7 digit number'),
            GovukValidators.max(9999999, 'Enter a 7 digit number'),
            GovukValidators.notNaN('Enter a 7 digit number'),
          ],
        ],
      }),
    ),
        ...
        switchMap(([form, account]) =>
          form.dirty
            ? this.accountUpdateService
                .updateAccountRegistryIdUsingPOST(account.id, {
                  registryId: form.get('registryId').value,
                })
                .pipe(tap((this.route.snapshot.data.account.registryId = form.get('registryId').value)))
            : of(null),
        ),
      )
