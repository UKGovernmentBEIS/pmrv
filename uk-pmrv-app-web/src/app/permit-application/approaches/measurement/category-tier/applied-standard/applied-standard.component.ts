...
export class AppliedStandardComponent implements PendingRequest {
    ...
    combineLatest([this.route.data, this.index$, this.store])
      .pipe(
        ...
        switchMap(([data, tierIndex, state]) =>
          this.store.findTask<MeasSourceStreamCategoryAppliedTier[]>(data.taskKey).pipe(
            first(),
            switchMap((tiers) =>
              this.store.postTask(
                data.taskKey,
                ...
                state.permitSectionsCompleted.MEASUREMENT_Applied_Standard.map((item, idx) =>
  ...
  private isNewAppliedStandard(tierIndex: number, tiers: MeasSourceStreamCategoryAppliedTier[]) {
